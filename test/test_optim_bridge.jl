...
#
# Tests for the `scipy(m)` / `minimize_with(m)` alternative-minimizer bridge
...
@testset "scipy / minimize_with Optim bridge" begin
    ...
    @testset "correlated quadratic: hesse recovers off-diagonal covariance" begin
        ...
        m = Minuit(f, [0.0, 0.0]); scipy(m; method = :lbfgs); hesse(m)
        @test m.values ≈ m_ref.values atol = 1e-5
    ...
    @testset "Newton converges unbounded (rejected only when bounded)" begin
        ...
        m = Minuit(f, [0.0, 0.0]); scipy(m; method = :newton)
    ...
    @testset "method-name aliases + optimizer-object passthrough" begin
        ...
        for meth in (:bfgs, :lbfgs, "L-BFGS-B", :conjugategradient, :gradientdescent)
            m = Minuit(f, [0.0, 0.0]); scipy(m; method = meth)
    ...
    @testset "ncall / tol / options knobs are accepted" begin
        ...
        m = Minuit(f, [0.0, 0.0]); scipy(m; method = :lbfgs, ncall = 500, tol = 1e-10)
        @test m.values ≈ [2.0, -3.0] atol = 1e-5
        m2 = Minuit(f, [0.0, 0.0])
        scipy(m2; method = :lbfgs, options = Optim.Options(g_tol = 1e-12))
        @test m2.values ≈ [2.0, -3.0] atol = 1e-6
    end

    @testset "resume: scipy starts from the current values" begin
        # A prior MIGRAD leaves m at the minimum; scipy from there stays put.
    ...
    @testset "Optim-not-loaded → helpful error (subprocess)" begin
        ...
        # Optim and confirm scipy(m) throws a 'load Optim' message — not a bare
        # MethodError.
        code = """
        using JuMinuit
