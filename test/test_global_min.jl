...
@testset "find_global_minimum (basin-hopping global search)" begin
    ...
    fmg = find_global_minimum(g, [2.0, 0.0], [0.5, 0.5];
                              n_restarts = 40, perturb = 1.0, seed = 3)
    @test JuMinuit.is_valid(fmg)
    @test fmg.state.parameters.x[1] > 0         # converged in the valid domain

    # argument validation
    @test_throws ArgumentError find_global_minimum(f, [0.0], [0.1]; n_restarts = 0)
    @test_throws ArgumentError find_global_minimum(f, [0.0], [0.1]; perturb = 0.0)
    @test_throws ArgumentError find_global_minimum(f, [0.0], [0.1]; max_rounds = 0)
    @test_throws ArgumentError find_global_minimum(f, [0.0], [0.1]; min_improvement = -1.0)
end
